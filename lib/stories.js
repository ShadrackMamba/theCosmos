...
async function parseSection(raw) {
  ...
  // Detect inner YAML block: starts with --- and ends with ---
  // Works regardless of what comes before the opening ---
  ...
  if (firstDash !== -1) {
    ...
    if (secondDash !== -1) {
      // Parse YAML lines between the two ---
      const yamlLines = lines.slice(firstDash + 1, secondDash)
      yamlLines.forEach(line => {
        // Split on first colon only — handles URLs (https://...) and
        // headings with em dashes, commas, and other special characters
      ...
      body = lines.slice(secondDash + 1).join('\n').trim()
    }
  }
